...
function isDict(v: unknown): v is any {
  return (
    typeof v === 'object' &&
    v !== null &&
    !(v instanceof Array) &&
...
export function serializePath(path: [number, number, number, number, number]) {
...
export function getVersion(transport: Transport): Promise<VersionResponse> {
  return transport
...
export function getAppInfo(transport: Transport): Promise<AppInfoResponse> {
  return transport
    .send(0xb0, 0x01, 0, 0)
    .then((response) => {
      const errorCodeData = response.slice(-2);
...
export function getAddressAndPubKey(
  transport: Transport,
  data: Buffer,
): Promise<PublicKeyResponse> {
  return transport
...
export function showAddressAndPubKey(
  transport: Transport,
  data: Buffer,
): Promise<PublicKeyResponse> {
  return transport
